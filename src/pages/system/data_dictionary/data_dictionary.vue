...
<template>
	<div class="journal">
			...
			<a-col :span="19">
				<a-card>
					<div class="journalList">
						<div class="flex flex-space-between borderbottom">
							<div>
								<a-button class="add mr10" type="primary" @click="addModify('add')"
									><template #icon><PlusOutlined /></template>新增</a-button
								>
								<a-button class="btn mr10" :disabled="disabled" @click="use('q')">启用</a-button>
								<a-button class="btn mr10" :disabled="disabled" @click="use()">停用</a-button>
							</div>
							<div class="flex">
						...
						<div style="margin-top: 30px">
							<a-table key="columns" :columns="columns" :data-source="journalList" :pagination="false" :row-selection="rowSelection">
								<template #status="{ record }">
									<span>
										<a-tag :color="record.status == 1 ? 'green' : 'volcano'">{{ record.status == 1 ? '在用' : '停用' }}</a-tag>
									</span>
								</template>
		...
		<a-modal v-model:visible="visible" :title="title" width="560px" @ok="handleOk" @cancel="cancelBtn">
			<a-form ref="roleRef" :model="roleForm" :rules="roleRule" :label-col="labelCol" :wrapper-col="wrapperCol">
				<a-form-item required label="编号" name="code">
					<a-input v-model:value="roleForm.code" placeholder="请输入编号" />
				</a-form-item>
	...
	setup() {
		...
		const rowSelection = {
			onChange: (selectedRowKeys: (string | number)[], selectedRows: []) => {
				...
				disabled.value = multipleSelection.value.length != 0
